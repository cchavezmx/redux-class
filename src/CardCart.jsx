export default function CardCart({ producto, removeToCart }){
  const { nombre, imagen, cantidad } = producto
        ...
        <div>
          <div className="card_product">
            <img 
              ...
              alt="producto" 
            />
          <h2 className="text-4xl py-2">{nombre}</h2>
          </div>
        </div>
        <div className="flex flex-col items-end">
          <button
            onClick={() => removeToCart(producto)} 
          ...
          <span className="p-2 bg-gray-500 rounded-md w-fit">
            ...
            <input className="text-2xl w-[50px] rounded bg-gray-500 text-gray-50" 
              type="number" defaultValue={cantidad} />
          </span>
        </div>
      </div>
  )
